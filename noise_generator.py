# Taken from Causal Component Analysis
# https://github.com/akekic/causal-component-analysis/blob/main/data_generator/noise_generator.py
...
class MultiEnvNoise(ABC):
    ...
    def __init__(
        ...
        assert shift_type in [
            "mean",
            "std",
        ], f"Invalid shift type: {shift_type}"
...
class GaussianNoise(MultiEnvNoise):
    ...
    def sample(self, env: int, size: int = 1) -> Tensor:
        return torch.normal(
            self.means_per_env[env].unsqueeze(0).repeat(size, 1),
            self.stds_per_env[env].unsqueeze(0).repeat(size, 1),
        )

    def log_prob(self, u: Tensor, env: int) -> Tensor:
        return torch.distributions.Normal(
            self.means_per_env[env].unsqueeze(0).repeat(u.shape[0], 1),
            self.stds_per_env[env].unsqueeze(0).repeat(u.shape[0], 1),
        ).log_prob(u)
